...
private:
	...
	std::array<SDL_Surface*, 5> backgroundImages;
	...
	void fillBackground(int scrollingOffset = 0);
	...
	void drawCustomWorldThankYouScreen(World &world, int level, int bgScrollingOffset = 0);
	void drawThankYouInscriptions(int i);
	void drawThankYouScreen(World &world, int bgScrollingOffset = 0);
	int initGUI();
	void closeGUI();
...
public:
	...
	void updateScreen(World &world, int bgScrollingOffset = 0);
	~Screen();
};
