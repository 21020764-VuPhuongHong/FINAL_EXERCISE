...
int Screen::computeTime() const
{
	...
	int time = static_cast<int>(initialTime - std::chrono::duration_cast<std::chrono::seconds>(timePoint - timeBegin).count());
...
void Screen::loadBackgroundImages()
{
	backgroundImages[0] = loadPNG("img/bg_imgs/background_openWorld.png", display);
	backgroundImages[1] = loadPNG("img/bg_imgs/background_underground.png", display);
	backgroundImages[2] = loadPNG("img/bg_imgs/background_castle.png", display);
	backgroundImages[3] = loadPNG("img/bg_imgs/background_winter.png", display);
	backgroundImages[4] = loadPNG("img/bg_imgs/background_summer.png", display);
}
...
void Screen::fillBackground(int scrollingOffset)
{
	if (World::LAYOUT_STYLE == LayoutStyle::OpenWorld) {
		drawSurface(display, backgroundImages[0], scrollingOffset + 320, 240);
		drawSurface(display, backgroundImages[0], scrollingOffset + 320 + SCREEN_WIDTH, 240);
	}
	else if (World::LAYOUT_STYLE == LayoutStyle::Underground) {
		drawSurface(display, backgroundImages[1], scrollingOffset + 320, 240);
		drawSurface(display, backgroundImages[1], scrollingOffset + 320 + SCREEN_WIDTH, 240);
	}
	else if (World::LAYOUT_STYLE == LayoutStyle::Castle) {
		drawSurface(display, backgroundImages[2], scrollingOffset + 320, 240);
		drawSurface(display, backgroundImages[2], scrollingOffset + 320 + SCREEN_WIDTH, 240);
	}
	else if (World::LAYOUT_STYLE == LayoutStyle::CustomWinter) {
		drawSurface(display, backgroundImages[3], scrollingOffset + 320, 240);
		drawSurface(display, backgroundImages[3], scrollingOffset + 320 + SCREEN_WIDTH, 240);
	}
	else if (World::LAYOUT_STYLE == LayoutStyle::CustomSummer) {
		drawSurface(display, backgroundImages[4], scrollingOffset + 320, 240);
		drawSurface(display, backgroundImages[4], scrollingOffset + 320 + SCREEN_WIDTH, 240);
	}
	else {
		setBlackBackground();
	}
...
void Screen::drawCustomWorldThankYouScreen(World &world, int level, int bgScrollingOffset)
{
	for (int i = 0; i < 1000; ++i) {
		fillBackground(bgScrollingOffset);
		world.draw(display);
		drawScreenElements();
		drawTime(0);
...
void Screen::updateScreen(World &world, int bgScrollingOffset)
{
