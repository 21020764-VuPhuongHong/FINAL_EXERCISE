...
//getX(), getY() cho ra toa do x, y o tam cua object
...
bool isCharacterStandingOnSomething(const WorldObject &object, const World &world)
{
	...
	for (auto &block : blocks) {
		if (isElementDirectlyAboveObject(object, block) && areAtTheSameWidth(object, block) && !block.isInvisible()) {
			return true;
		}
	}
...
bool isPlayerCloseToFireRocket(const FireMissile &fireRocket, const World &world)
{
...
//Khi npc bi pha huy, hien thi diem cong phia tren npc
...
//Quan ly viec xoa fireball duoc ban ra tu Fire Mario
...
//Khi Mario va cham voi npc ma khong phai la nhay len tieu diet npc
...
//Quan ly va cham giua shell va npc --> shell pha huy npc
...
//Fireball tu Fire Mario lam giam sut suc manh cua cac npc 
...
//Npc va cham voi block khi dang dung tren block do --> npc bi pha huy 
...
//Vat pham bonus (ngoai tru coin) va cham voi block khi dang dung tren block do --> hien thi hieu ung vat pham bonus nhay len 
...
//Khi Mario thu thap duoc vat pham bonus
