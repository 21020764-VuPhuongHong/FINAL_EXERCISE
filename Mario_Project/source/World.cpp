...
void World::performFireBallsActions()
{
	for (std::size_t i = 0; i < fireballs.size(); ++i) {
		...
		if (fireballs[i].shouldBeRemoved()) {
			...
			temporaryElements.push_back(std::make_shared<Explosion>
														(Explosion(Position(fireballs[i].getX() + shift, fireballs[i].getY()))));
...
{
	npcs.push_back(std::make_shared<FireMissile>(FireMissile(position, MissileType::Bomb)));
}
